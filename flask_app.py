...
@app.route('/google_drive/status', methods=['GET'])
def google_drive_status():
    """Check Google Drive authentication status"""
    try:
        drive_manager.load_credentials()
...
@app.route('/google_drive/authorize', methods=['GET'])
def google_drive_authorize():
    ...
    try:
        ...
        flash('Google Drive credentials.json not found. Please see setup instructions.', 'error')
        return redirect(url_for('index'))
    except Exception as e:
...
@app.route('/google_drive/callback', methods=['GET'])
def google_drive_callback():
    ...
    try:
        ...
        if drive_manager.authenticate_from_code(code, redirect_uri, state):
...
@app.route('/google_drive/revoke', methods=['POST'])
def google_drive_revoke():
    """Revoke Google Drive access"""
    try:
        drive_manager.revoke_authentication()
        flash('Google Drive access revoked successfully.', 'success')
    except Exception as e:
        flash(f'Failed to revoke access: {str(e)}', 'error')
    
    return redirect(url_for('index'))


@app.route('/export_to_drive', methods=['POST'])
def export_to_drive():
    """Export current report as PDF to Google Drive"""
    data = session.get('last_result')
    if not data:
        return jsonify({'success': False, 'error': 'No report found to export. Please run a prediction first.'})
    ...
    try:
        # Generate PDF (same logic as export_pdf route)
        pdf_io = BytesIO()
        doc = SimpleDocTemplate(pdf_io, pagesize=A4, leftMargin=18*mm, rightMargin=18*mm, topMargin=18*mm, bottomMargin=18*mm)
        styles = getSampleStyleSheet()
        story = []
        ...
        filename = (data.get('patient_id') or 'ecg_report') + '_' + datetime.now().strftime('%Y%m%d_%H%M%S') + '.pdf'
        result = drive_manager.upload_file(pdf_io, filename, mimetype='application/pdf')
        
        if result.get('success'):
            return jsonify({
                'success': True,
                'message': f'Report uploaded successfully to Google Drive!',
